...
const faqs = [
    ...
    {
        category: "Xtream Code Generator",
        questions: [
            {
    ...
    {
        category: "Stalker Code Generator",
        questions: [
            {
        ...
        questions: [
            {
                ...
                answer: "Slow loading times can result from various factors: (1) Check your internet connection speed and stability. (2) Clear your browser cache and cookies, which can accumulate and slow performance. (3) Try using a different browser or updating your current browser to the latest version. (4) Disable unnecessary browser extensions that might interfere with page loading. (5) If you're on a mobile device, ensure you have adequate signal strength. We continuously optimize our platform for speed, but these steps can help resolve client-side performance issues."
            },
            {
                question: "Can I use IPTV Factory on mobile devices?",
                answer: "Absolutely! IPTV Factory is fully responsive and optimized for mobile devices, including smartphones and tablets. Our interface adapts seamlessly to different screen sizes, ensuring a smooth experience whether you're on iOS or Android. All features—including code generation, downloading credentials, and sharing options—work perfectly on mobile browsers. For the best experience, we recommend using updated versions of Chrome, Safari, Firefox, or Edge on your mobile device."
            },
            {
                question: "How do I download my generated codes?",
                answer: "We provide multiple convenient download options for your generated codes: (1) TXT File: Download credentials as a plain text file that can be opened on any device. (2) Image/Screenshot: Save your codes as a PNG image for easy visual reference. (3) Copy to Clipboard: Instantly copy all credentials with a single click. (4) Share Function: Use your device's native sharing capabilities to send codes via email, messaging apps, or cloud storage. These options make it easy to transfer credentials to different devices or keep backup copies."
            }
        ]
    }
];
...
const FAQItem = ({ question, answer, isOpen, onClick }) => {
    return (
        <div className="border-b border-gray-200 last:border-b-0">
            <button
                onClick={onClick}
                className="w-full py-5 px-6 flex justify-between items-center text-left hover:bg-gray-50 transition-colors duration-200 group"
            >
                <span className="text-lg font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors pr-8">
                    {question}
                </span>
                {isOpen ? (
                    <ChevronUpIcon className="h-6 w-6 text-indigo-600 flex-shrink-0" />
                ) : (
                    <ChevronDownIcon className="h-6 w-6 text-gray-400 group-hover:text-indigo-600 transition-colors flex-shrink-0" />
                )}
            </button>
            {isOpen && (
                <div className="px-6 pb-6 text-slate-700 leading-relaxed animate-fadeIn">
                    {answer}
                </div>
            )}
        </div>
